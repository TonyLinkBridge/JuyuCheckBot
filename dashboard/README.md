...
包含新用户激活、订阅解锁、推荐增长、聚查导流、商业 Leads、来源质量和体检数据健康监控。质量区显示权威资料率、注册状态确认率，并按注册资料源列出报告量与成功率，不再显示 JUYU 自创结构分。Referral Funnel 会追踪分享生成、推荐打开、推荐新用户、域名提交与报告解锁，并显示 K-factor 和表现最佳的分享域名。Business Conversion 会同时显示进入聚查的用户，以及购买、出售与注册意向进入 `@JuyuDomainBot` 后的完成率。`/polls` 提供受保护的 Telegram Poll 发布器，可先发测试频道，再确认发布到正式频道，并自动生成 `src_` 来源链接。
