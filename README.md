...
- 版本化 `JUYU-1.2` 六维评分、等级、置信度、数据覆盖和基础风险提醒
...
`JUYU-1.2` 包含品牌力、记忆度、商业潜力、后缀匹配、全球化能力与活跃度信号。当前仍是透明的初筛启发式规则，数据来自 RDAP、DNS、域名结构和基础发音规则；外部数据不完整时，对应活跃度维度显示 `N/A`、不参与总分，并通过低置信度提示用户。商业潜力与活跃度尚未包含成交数据库。它不是域名估值，也不代表商标可用性、历史内容安全、SEO 信誉或交易合法性。任何权重变更都应发布新的 Score Version。
